...
oc project pagerduty-ldap-sync
if [[ $? -ne 0 ]] ; then
  echo "You need to login to openshift: "
  oc login `echo $OPENSHIFT_URL`
  oc project pagerduty-ldap-sync
fi
echo "oc is set to project pagerduty-ldap-sync"
docker build -t pagerduty-ldap-sync .
if [[ $? -ne 0 ]] ; then
...
docker tag pagerduty-ldap-sync $DOCKER_REGISTRY_URL/pagerduty_tools/pagerduty_ldap_sync
if [[ $? -ne 0 ]] ; then
  echo "Failed to tag the docker image."
  exit 1
fi
echo "Tagged docker image."
docker push $DOCKER_REGISTRY_URL/pagerduty_tools/pagerduty_ldap_sync
if [[ $? -ne 0 ]] ; then
...
oc deploy pagerduty-ldap-sync --latest=true
if [[ $? -ne 0 ]] ; then
  echo "oc deploy pagerduty-ldap-sync --latest=true HAS FAILED"
  exit 1
fi
sed -i.bak -e s/$DOCKER_REGISTRY_URL/'{DOCKER_REGISTRY_URL}'/g openshift_template.yml
rm openshift_template.yml.bak
echo ""
echo "Successfully deployed your docker image to openshift, `echo $OPENSHIFT_URL`/console/project/pagerduty-ldap-sync/overview?main-tab=openshiftConsole%2Foverview"
